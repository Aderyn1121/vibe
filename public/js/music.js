...
const playClickedSong = async (event) => {
    ...
    const songJSON = await fetch(`/api/songs/${songId}`, {
        headers: {
...
const updatePlaylists = async () => {
    const user = await getUser();
    const playlistsJSON = await fetch(`/api/users/${user.userId}/playlists`, {
        headers: {
            Authorization: `Bearer ${localStorage.getItem('VIBE_TOKEN')}`,
        },
    });
...
const addPlaylistNewPage = async () => {
    ...
    await fetch(`/api/users/${user.userId}/playlists`, {
        method: 'POST',
        headers: {
    ...
    for (let i = 0; i < playlistList.length; i++) {
        let attribute = Number.parseInt(
            playlistList[i].getAttribute('playlistid')
        );
...
const updateHome = async () => {
    ...
    const playlistsJSON = await fetch(`/api/users/${userId}/playlists`, {
        headers: {
            Authorization: `Bearer ${localStorage.getItem('VIBE_TOKEN')}`,
        },
    });
        ...
        if (section === 'songs') {
            ...
            plus.addEventListener('click', (event1) => {
                ...
                contextPlaylists.addEventListener(
                    'click',
                    async (event3) => {
                        const playlistId = event3.target.getAttribute(
                            'playlistid'
                        );
...
const updateSearch = async () => {
    ...
    const resultsJSON = await fetch(
        `/api/search/?searchInput=${searchInput}&userId=${userId}`,
        {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${localStorage.getItem('VIBE_TOKEN')}`,
...
const getPlaylist = async (playlistId) => {
    const playlistJSON = await fetch(`/api/playlists/${playlistId}`, {
        method: 'GET',
        headers: {
            Authorization: `Bearer ${localStorage.getItem('VIBE_TOKEN')}`,
        },
...
const getPlaylistSongs = async (playlistId) => {
    const songsJSON = await fetch(`/api/playlists/${playlistId}/songs`, {
        method: 'GET',
        headers: {
            Authorization: `Bearer ${localStorage.getItem('VIBE_TOKEN')}`,
        },
...
const editPlaylist = async () => {
    ...
    const updatePlaylistName = async (event) => {
        ...
        await fetch(`/api/playlists/${playlistId}/edit`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${localStorage.getItem('VIBE_TOKEN')}`,
